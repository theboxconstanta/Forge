...
# P9.5.2 — PERFORMED PRESCRIPTION + MEMBER EDIT

Owner **APPROVED** the additive `public.wod_logs.performed_prescription jsonb NULL`
contract (P9.5.1 §G). SCHEMA REVIEW → MIGRATION → IMPLEMENTATION → TESTS →
DEPLOY → REPORT → HARD STOP. **P10 remains NOT STARTED.**

## A. Schema review — SAFE, no §69 HARD-STOP condition triggered

| §69 stop condition | finding |
|---|---|
| RLS would need broadening | **No.** `wod_logs` policies are row-scoped: `wod_logs_insert_own` WITH CHECK `(member_id = auth.uid()) AND (gym_id = my_gym_id())`, `wod_logs_update_own` USING+CHECK `member_id = auth.uid()`. A member can only write their own log. Adding a column needs zero policy change. |
| Programmed snapshot semantics would change | **No.** `prescription_snapshot` (P9.1) and `movements_snapshot` (trigger-maintained) stay PROGRAMMED provenance, never written by the Edit flow. |
| Historical rows need backfill | **No.** Column is nullable, no default. All 412 live rows read `NULL` = "performed as programmed". Verified post-migration: `non_null_rows = 0`. |
| `weight_logged` conflicts irreconcilably | **No.** `weight_logged` stays the single-value legacy athlete weight (leaderboard / share popup / `greutateEsteSubStandard`). `performed_prescription` is the per-movement authoritative overlay. They do not compete: `isNotRxd` treats a non-null overlay as decisive, `weight_logged` still feeds the legacy comparison for un-overlaid logs. |
| RX/Modified cannot be determined safely | **No.** `performedIsModified()` compares the athlete-resolved performed doc against the programmed variant. The save path only persists the overlay when it *materially* differs; its mere presence then flips `isNotRxd` → Modified. The **variant is never changed** (RX stays RX). |
| Substitution cannot retain canonical identity | **No.** `applyPerformedSubstitution()` adopts the target catalog row's `id` as `canonicalMovementId` and records `substitutedFrom`. |
| Malformed performed data cannot be validated | **No.** `validate_wod_log_performed_prescription()` BEFORE trigger (mirrors `validate_movement_prescriptions()`) + client `validatePerformedPrescription()`. Fail closed. |

**Views:** `wod_logs_with_context`, `performance_timeline`, `athlete_performance_summary`,
`gym_performance_summary` all use explicit narrow column lists (never `SELECT *`) —
adding the column breaks none; row counts unchanged post-migration (412 / 33 / 37 / 4).
The client never reads `wod_logs_with_context`.

**Client reads:** `fetchWodLogs` (`.select('*, wods(...)')`), `fetchClasament`
(`.select('*')`), `workoutAggregationRead.js` — `*` auto-includes the new column.

## B. Migration — `supabase/migrations/20260831090000_wod_logs_performed_prescription.sql`

```sql
ALTER TABLE public.wod_logs ADD COLUMN IF NOT EXISTS performed_prescription jsonb;
COMMENT ON COLUMN public.wod_logs.performed_prescription IS '…';
CREATE OR REPLACE FUNCTION public.validate_wod_log_performed_prescription() … ;  -- SECURITY INVOKER
CREATE TRIGGER wod_logs_validate_performed_prescription
  ...
  FOR EACH ROW EXECUTE FUNCTION public.validate_wod_log_performed_prescription();
```

Additive only. Manual DOWN documented in the file. **NO backfill.**

## C. Dry-run (transaction + ROLLBACK against the linked prod DB)

`A1 column :: type=jsonb nullable=YES default=<none>` ·
`A2 backfill :: total=412 non_null=0` ·
`A3 universal load :: accepted` · `A4 sex_specific load :: accepted` ·
`A5 substitutedFrom + decimal :: accepted` ·
`A6 malformed rejected :: 8 / 8` (bad version, non-array movements, missing
instanceId, dup instanceId, bad load unit, non-numeric value, bad distance unit,
bad variantKey) · `A7 null clear :: accepted` ·
`A8 wod_logs_with_context :: 412` (+ 3 other views unchanged) ·
`A9 trigger present`, `fn security_definer :: false`.

## D. Applied to production

`supabase db query --linked -f 20260831090000_…sql` → verified live: column
`jsonb NULL`, `non_null_rows = 0`, trigger present, function SECURITY INVOKER,
comment present. **0 row mutations.**

## E. Contract (`prescriptionContract.js` ⇄ `.ts`, byte-parallel)

- `PERFORMED_PRESCRIPTION_VERSION = 1`, `PERFORMED_EDITABLE_METRICS = ['load','distance','calories']`
- `buildPerformedPrescriptionDraft({doc,variantKey,sectionId})` → deep VALUE clone of the frozen programmed variant's instances, tagged `{version, variantKey, sectionId, source:'performed', movements}`; `null` for a legacy variant.
- `validatePerformedPrescription(doc)` → `{valid,errors}`, mirrors the DB trigger; `NULL` valid.
- `performedMatchesProgrammed(perf, prog, variantKey, gender)` — athlete-resolved equality (identity + resolved load/distance/calorie/reps values). Drives "store NULL".
- `performedIsModified(...)` — the negation, tolerant of a missing programmed doc.
- `applyPerformedSubstitution(instance, targetRow, capability)` — keeps `instanceId`; adopts target identity; records `substitutedFrom` **once**; retains a load/distance/calorie spec only if the target still allows it; `reps` (structure) always carries over.
- `setPerformedMetricValue(instance, metric, value, unit)` — writes the athlete's own single performed value as `{mode:'universal', value, unit}`.

## F. Member Edit flow (WOD-SIMPLE, `logWodPrimaryPath` only)

- **Edit button** top-right of the Universal Log WOD screen. Shown only when
  `editEligible` = structured variant **AND** `memberGenderKey != null` **AND**
  a resolved variant key. Legacy free-text → no button (§42). **Unknown gender →
  no button** (§57 HARD STOP that path — never guess a sex-specific value).
- **Focused Edit mode** (`logWodEditMode`) replaces the score/save body with
  `<PerformedEditPanel>`: per movement — the locked reps line (read-only),
  a "Change movement" catalog search (canonical `id`, `substitutedFrom` note),
  and one numeric field per **present** load/distance/calorie metric
  (`<PmpeNumField>`, comma/dot P9.2, calories integer). No add/remove/reorder,
  no rounds, no format, no time cap.
- **Cancel** discards the draft. **Done** validates, then
  `setPerformedCommitted(performedMatchesProgrammed(...) ? null : draft)`.
- Back on the main screen: a **MODIFIED** chip by the variant badge; the
  read-only workout rows re-render from the performed overlay.
- **Local draft only** — nothing hits the backend until Save.
- `performedCommitted` / `performedDraft` / `logWodEditMode` reset on leaving the
  logger, on variant change, and after save. `onEditWod` seeds
  `performedCommitted` from `log.performed_prescription` (read-through; no
  structured re-edit UI on the edit-existing path in V1).

## G. Save semantics

`saveWodLog` primary insert: `performedToSave = (performedCommitted &&
performedIsModified(performedCommitted, logCtx.prescriptionDoc, variantKey,
gender) && validatePerformedPrescription(performedCommitted).valid)
? performedCommitted : null`; then `...(performedToSave ? {performed_prescription:
performedToSave} : {})`. Atomic with the existing single `.insert`. The
edit-existing `.update` never names the column → the overlay is preserved.

## H. RX / Modified classification (§26–§28)

`isNotRxd(log, …)` gains `|| log?.performed_prescription != null`. The share
popup passes `{...logFields, performed_prescription: performedToSave}`. The
variant stays RX; only the RX→Modified/Not-RX flag flips. Journal / leaderboard
read the same `isNotRxd` → consistent everywhere. **Leaderboard sorting is
unchanged in P9.5.2** (§35).

## I. Tests — `src/performedPrescription.test.js` (20)

draft build + deep-clone isolation + legacy null; validation (null, well-formed,
bad version/array/instanceId/dup/unit/value/variantKey); match/modified
(untouched clone, changed load, revert-to-resolved-value, substitution, null);
substitution (instanceId stable, identity adopted, `substitutedFrom` once,
incompatible metric cleared, unknown-capability retains data); repeated movements
no cross-contamination (§32); distance enum; `isNotRxd` reads the column.
Full suite **1300 passed** (+ the 9 pre-existing Deno `@std/assert` file-resolution
failures, unrelated). `forge-admin-web`: `tsc -p tsconfig.app.json` clean,
`src/features/programming` **483 passed**.

## J. Non-goals confirmed untouched (§67)

Format / round count / reps-per-movement / add-remove-reorder movements / time
cap / score type / section structure editing; legacy free-text movement editing;
section & skill flows (P9.5 guard intact — scope is the programmed primary
metcon); leaderboard sorting; performance analytics; snapshot-first historical
reads (P10).

## K. Manual acceptance checklist (owner)

1. Structured RX metcon, known gender → **Edit** appears top-right; legacy
   free-text workout → no Edit; account with no gender set → no Edit.
2. Edit → lower one movement's load → **Done** → **MODIFIED** chip; workout rows
   show the new load; Save → Journal entry shows **Not Rx**.
3. Re-open that log (Journal → edit) → still **Not Rx**; changing the note and
   re-saving keeps the overlay.
4. Edit → change a value then change it back to the programmed value → **Done**
   → no chip, saves with `performed_prescription = NULL`.
5. Edit → "Change movement" → pick a substitute → "Instead of X" note; a metric
   the new movement can't take is dropped; reps unchanged.
6. Repeated movement (e.g. 2× Run): edit the first only → the second stays as
   programmed.
7. Time Capped result + an Edit → both persist; leaderboard order unchanged.
8. Cancel in Edit mode → no chip, nothing saved.

## HARD STOP

**P10 NOT STARTED.** `performed_prescription` is written by the member Edit flow
