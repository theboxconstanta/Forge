...
**Audience:** Engineers working on Forge, present and future. Read this before touching authentication, authorization, Edge Functions running with `service_role`, or any code path in the production-critical areas listed below.
...
Every fix was reviewed and explicitly approved by the project owner before any deployment occurred, including a distinct approval step for "investigate" versus "implement" versus "deploy." **Why:** this project treats subscriptions, billing, membership expiration, and access control as production-critical by explicit standing rule (see `docs/DECISIONS.md` and this document's Section 7); changes reaching those areas require a human decision point, not an autonomous one.
...
| P0-003 | `admin-delete-client` cross-tenant deletion | P0 (Critical — Authorization / Multi-Tenant Isolation) | Edge Function runs with `service_role` (bypasses RLS) and verified caller was *an* admin, but never verified the admin's gym matched the target member's gym | Fixed | Live (v4) | Deployed via `supabase functions deploy`, validated with production smoke test |
| P0-004 | `send-notification` missing authorization | P0 (Critical — Authorization) | Edge Function runs with `service_role`; platform-level `verify_jwt` only confirmed *a* valid session existed, but the function itself performed zero identity or authorization checks before sending real email/push notifications | Fixed | Live (v8) | Deployed via `supabase functions deploy`, validated with production smoke test |
...
- `admin-delete-client` and `send-notification` now establish caller identity server-side via `anonClient.auth.getUser(token)` before performing any privileged action, matching the pattern already used correctly in `analyze-workout` prior to this cycle.
- In both `admin-delete-client` and `send-notification`, authorization is evaluated and enforced before any database write, cascading deletion, external API call, or notification send — verified by static code review and, in production, by confirming rejected requests produce no observable side effect (no email sent, no push sent, no `push_subscriptions` cleanup, no rows modified).
- Cross-tenant member deletion via `admin-delete-client` is prevented: an admin can no longer delete a member of a gym other than their own. Verified in production.
- Unauthorized notification sends via `send-notification` are prevented: a non-staff caller can no longer send a notification to an arbitrary member or an arbitrary gym, with one narrowly-scoped, verified exception (`waitlist_booked`, same-gym only) required by an existing legitimate workflow. Verified in production.
- The specific privilege-escalation paths described in P0-003 and P0-004 (cross-tenant deletion via `admin-delete-client`; cross-tenant and role-agnostic notification sending via `send-notification`) are eliminated.
...
- *Reason it remains:* identified during the same audit that produced P0-003 and P0-004 (as "Finding 2"), but this cycle's implementation work was scoped to P0-003 and P0-004 only; a dedicated investigation-and-fix cycle for this function has not yet been requested or performed.
...
- *Reason it remains:* identified during the audit as "Finding 4," confirmed exploitable, but not yet investigated for a fix. It sits directly in the production-critical `subscriptions` table, which requires explicit approval before any change per standing project policy.
