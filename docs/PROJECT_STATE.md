...
12. **M8 (Membership Catalog / First Purchase) — CLOSED, 2026-07-22 through 2026-07-24.** Scoped 2026-07-21 as new, deferred work — a brand-new member with zero subscription history had no self-service purchase path ("Renew Now" only renewed an existing plan). Implemented the next day (`1aedf10`, "finalize membership catalog and first purchase flow") and hardened over the following two days: admin deletion of never-used plans, subscription-plan field validation, realtime publication for the catalog tables, and subscription/plan action-terminology alignment. The member-facing Catalog screen unifies first purchase and renewal into one entry point ("Renew Now" was withdrawn in favor of it), calling the same `create-checkout-session`/Stripe/webhook infrastructure M6 already validated — no new backend was needed. Predates, and is unaffected by, the Member Domain Migration above (entirely a Financial Domain + `profiles`-email-keyed feature, never coupled to `members`). This closure went unrecorded in `Current Open Work` and `docs/ROADMAP.md` until now — a documentation lapse independent of and not caused by the Member Domain Migration's own documentation sync.
13. **Gym Transfer (M7) — CLOSED, 2026-07-27.** Built on the frozen Member Domain (#11): an admin can end a Membership as a Transfer (`memberships.status = 'transferred'`) rather than a Removal; the member later joins any gym through the existing, unmodified self-service join, with Forge recognizing the continuity as a read-only derivation over Membership history, never a stored fact. A new `transfer_codes` entity covers the fallback case where the member cannot complete that join themselves — a 72-hour, single-use, admin-issued/admin-redeemed credential, with a partial unique index enforcing exactly one active code per transfer under concurrent requests. Preceded by a full, staged architecture cycle (Product Specification, Technical Architecture, Implementation/Execution Plans, Freeze Notice — all now persisted as canonical documents, not left as conversation-only artifacts) before any of the 11 implementation steps began, each independently reviewed, validated against the live database, and committed. Two new Edge Functions (`admin-transfer-member`, `redeem-transfer-code`), 11 migrations, zero regression found in Remove Member or the self-service join path. See `docs/architecture/M7_FREEZE_NOTICE.md` and `docs/DECISIONS.md`.
...
| `admin-transfer-member` | Ends a member's relationship with a gym as a Transfer rather than a Removal (`memberships.status = 'transferred'`), same effects otherwise — sibling to `admin-remove-member`, reuses its authorization function verbatim | Caller must be an admin of the target's own gym; cannot target another admin |
| `redeem-transfer-code` | Fallback Gym Transfer redemption: a destination admin completes an enrollment on a member's behalf using a Transfer Code | Caller must be an admin (any gym); the code itself resolves which member is joining, validated inside `redeem_transfer_code` |
...
- Gym Transfer (M7) is closed as of 2026-07-27: `memberships.status = 'transferred'` (reserved since M1.1) is now populated for the first time; `transfer_codes` is a new, Gym-Transfer-owned table with no INSERT/UPDATE RLS policy at all (writes exist only through `end_membership_as_transfer`/`issue_transfer_code`/`revoke_transfer_code`/`redeem_transfer_code`, all service-role- or is_admin()-gated). Every step validated against the live production database inside rolled-back transactions, never a separate staging environment. See `docs/architecture/M7_FREEZE_NOTICE.md` and `docs/DECISIONS.md`.
