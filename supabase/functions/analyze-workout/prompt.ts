...
- AMRAP (structure "Repeated Rounds" sau nespecificat), Ascending AMRAP, AMRAP with Buy-In -> "Rounds + Reps"
- AMRAP cu structure "Sequence" -> "Reps" (total repetari pe parcursul ordonat)
...
- formatConfig.timeCapMinutes = limita de timp a sectiunii (daca exista); formatConfig.rounds/intervalSeconds/workSeconds/restSeconds/startReps/incrementReps se completeaza doar cand formatul respectiv le foloseste (EMOM -> intervalSeconds, Tabata -> workSeconds+restSeconds+rounds, Death By -> startReps+incrementReps+intervalSeconds) - restul raman null, nu 0
- formatConfig.structure (DOAR pt "AMRAP" si "For Time"): "Sequence" cand munca e o SINGURA parcurgere ordonata a unei liste finite de sarcini - un "buy-in" / chipper (ex. "AMRAP 10: 50 Burpee Pull-ups, 75 KB Swings, apoi Max Reps Burpee Pull-ups"), NU o runda care se repeta. "Repeated Rounds" cand aceleasi miscari se reiau pana expira timpul (ex. "AMRAP 12: 5 Pull-ups, 10 Push-ups, 15 Squats"). Daca nu e clar, lasa null (se trateaza ca "Repeated Rounds"). NU marca miscarile individual ca "fixed"/"open" - statia deschisa (max reps) e cea FARA numar de reps prescris ("reps": null), o deducem noi din asta.
- INTERVALS (INC-07): daca WOD-ul are STATII distincte scorabile per runda (stil "Fight Gone Bad" - de ex. "5 Rounds: :40 HSPU / :20 Rest / :40 Renegade Row / :20 Rest / :40 Shuttle / :20 Rest"), completeaza formatConfig.roundCount = NUMARUL REAL de runde repetate (aici 5, NICIODATA 5×3=15), formatConfig.stationMode = "per-interval", formatConfig.workSeconds + formatConfig.restSeconds (durata unei statii / a pauzei), si pune fiecare STATIE scorabila (fara "Rest") in "movements". formatConfig.rounds ramane null (il derivam noi = roundCount × nr. statii). Pauza NU e niciodata o miscare - nu o pune in "movements". Daca structura runde/statii nu e clara din text, lasa roundCount null si stationMode null (fallback pe modelul vechi cu rounds) - nu inventa.
- formatConfig.setsScheme (DOAR pt format "Strength Sets"): schema de seturi ca listă PLATĂ de reps, un număr per set (nu grupată) - ex. "2 seturi x 5 reps, 3 seturi x 4 reps, 2 seturi x 3 reps" -> [5,5,4,4,4,3,3] (2 cincii, 3 patrii, 2 trei); "5x5" -> [5,5,5,5,5]. NU scrie schema de seturi in "reps"-ul miscării (acela rămâne null pt Strength Sets - schema întreagă trăiește DOAR în formatConfig.setsScheme) și NU o repeta în "notes" - numele mișcării rămâne curat (ex. "Snatch"), niciodată propoziția întreagă ("Snatch (2 seturi x 5 reps...)"). Dacă schema de seturi nu e clară din text, lasă formatConfig.setsScheme gol [] (nu inventa) - secțiunea rămâne needs-review în loc să primească o schemă ghicită. weightMale/weightFemale rămân null dacă nicio greutate fixă nu e specificată explicit - un Strength Sets fără greutate prescrisă (fiecare atlet își alege greutatea) e un rezultat valid, nu un semn că lipsește ceva.
- formatConfig.targetRepMax (DOAR pt format "Build to Heavy/1RM"): numărul de repetări al testului, ca INTEGER (1-30) - ex. "Build to a 3-rep-max Front Squat" -> 3, "Build to a heavy 5RM Snatch" -> 5, "Build to a heavy single Clean & Jerk" / "build to a heavy double Deadlift" -> single=1, double=2, triple=3. Numele mișcării rămâne curat (ex. "Front Squat"), NICIODATĂ propoziția întreagă ("Build to a 3-rep-max front squats") - descrierea rep-max-ului trăiește STRICT în formatConfig.targetRepMax, nu în "notes" și nu în numele mișcării. Dacă textul spune doar "build to heavy" / "build to a heavy X" fără niciun număr de repetări explicit sau deductibil, lasă formatConfig.targetRepMax null (nu presupune 1RM) - secțiunea folosește atunci default-ul editorului (1RM), la fel ca la autoring manual.
